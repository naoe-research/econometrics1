...
推定式 (2) は以下のように表される：
$$
\text{Divorce Rate}_{s,t} = \sum_{k=1}^{K} \beta_k \cdot \text{(U } k \text{ periods)}_{s,t} + \sum_{s} \gamma_s \cdot \text{(State fixed effects)}_s + \sum_{t} \delta_t \cdot \text{(Time fixed effects)}_t + \sum_{s} \phi_s \cdot (\text{State} \times \text{Time}) + \sum_{s} \psi_s \cdot (\text{State} \times \text{Time}^2) + \epsilon_{s,t} 
$$
